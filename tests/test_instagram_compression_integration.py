...
class TestInstagramCompressionIntegration:
    ...
    async def test_large_video_compression_success(self, mock_get_size, mock_should_compress, 
                                                  mock_compressor_class, mock_get_info, 
                                                  mock_glob, mock_instaloader):
        ...
        await process_instagram(self.mock_message, self.mock_bot, instagram_url, self.mock_progress_msg)
        
        # Verify compression was attempted
        mock_compressor.compress_if_needed.assert_called_once_with(self.test_video_path, max_size_mb=50.0)
        ...
        expected_calls = [
            ...
            call("✅ Video compressed from 60.0MB to 45.0MB\n📉 Size reduced by 25%\n⏳ Sending video..."),
    ...
    @patch('handlers.social_media.instagram.should_compress_video')
    @patch('handlers.social_media.instagram.get_file_size_mb')
    async def test_compression_failure_fallback(self, mock_get_size, mock_should_compress, 
                                              mock_compressor_class, mock_get_info, 
                                              mock_glob, mock_instaloader):
        ...
        fallback_calls = [call for call in self.mock_progress_msg.edit_text.call_args_list 
    ...
    @patch('handlers.social_media.instagram.should_compress_video')
    @patch('handlers.social_media.instagram.get_file_size_mb')
    async def test_small_video_no_compression(self, mock_get_size, mock_should_compress, 
                                            mock_get_info, mock_glob, mock_instaloader):
        ...
        size_info_calls = [call for call in self.mock_progress_msg.edit_text.call_args_list 
    ...
    @patch('handlers.social_media.instagram.should_compress_video')
    @patch('handlers.social_media.instagram.get_file_size_mb')
    async def test_compression_error_handling(self, mock_get_size, mock_should_compress, 
                                            mock_compressor_class, mock_get_info, 
                                            mock_glob, mock_instaloader):
        ...
        mock_compressor_class.return_value = mock_compressor
        mock_compressor.estimate_compression_time.return_value = 30
        mock_compressor.compress_if_needed.side_effect = Exception("Compression service unavailable")
        ...
        error_calls = [call for call in self.mock_progress_msg.edit_text.call_args_list 
    ...
    @patch('handlers.social_media.instagram.glob.glob')
    @patch('handlers.social_media.instagram.get_video_info')
    async def test_progress_message_updates_sequence(self, mock_get_info, mock_glob, mock_instaloader):
        ...
        with patch('handlers.social_media.instagram.get_file_size_mb', return_value=25.0), \
             patch('handlers.social_media.instagram.should_compress_video', return_value=False):
            ...
            size_info_found = any("Video size: 25.0MB (within limit)" in text for text in call_texts)
            assert size_info_found, "Size info should be included in progress messages"
            
            # Should end with success message
            success_found = any("✅ Instagram video processed successfully!" in text for text in call_texts)
...
class TestInstagramCompressionProgressMessages:
    ...
    @patch('handlers.social_media.instagram.should_compress_video')
    @patch('handlers.social_media.instagram.get_file_size_mb')
    async def test_compression_progress_message_sequence(self, mock_get_size, mock_should_compress, 
                                                       mock_compressor_class, mock_get_info, 
                                                       mock_glob, mock_instaloader):
        ...
        for pattern in expected_patterns:
            ...
            assert found, f"Expected progress message pattern '{pattern}' not found in: {call_texts}"
    ...
    @patch('handlers.social_media.instagram.should_compress_video')
    @patch('handlers.social_media.instagram.get_file_size_mb')
    async def test_compression_timeout_progress_handling(self, mock_get_size, mock_should_compress, 
                                                       mock_compressor_class, mock_get_info, 
                                                       mock_glob, mock_instaloader):
        ...
        mock_compressor.compress_if_needed.side_effect = CompressionTimeoutError("Compression timed out")
        ...
        timeout_found = any("Compression failed: Compression timed out" in text for text in call_texts)
...
class TestInstagramCompressionCleanup:
    ...
    async def test_compressed_file_cleanup_after_success(self, mock_unlink, mock_get_size, mock_should_compress, 
                                                        mock_compressor_class, mock_get_info, 
                                                        mock_glob, mock_instaloader):
    ...
    async def test_temp_directory_cleanup_after_processing(self, mock_rmtree, mock_get_size, mock_should_compress, 
                                                          mock_compressor_class, mock_get_info, 
                                                          mock_glob, mock_instaloader):
    ...
    @patch('handlers.social_media.instagram.get_file_size_mb')
    @patch('handlers.social_media.instagram.shutil.rmtree')
    async def test_cleanup_on_exception(self, mock_rmtree, mock_get_size, mock_should_compress, 
                                       mock_compressor_class, mock_get_info, 
                                       mock_glob, mock_instaloader):
