...
class TestSocialMediaFallbackIntegration:
    ...
    @pytest.mark.asyncio
    async def test_successful_video_delivery(self, mock_bot, mock_message, mock_progress_msg, temp_video_file):
        ...
        result = await _attempt_video_delivery_with_fallbacks(
            mock_bot, mock_message, temp_video_file, platform_name, 
            original_url, file_size_mb, False, mock_progress_msg
        )
    ...
    @pytest.mark.asyncio
    async def test_document_fallback_delivery(self, mock_bot, mock_message, mock_progress_msg, temp_video_file):
        ...
        result = await _attempt_video_delivery_with_fallbacks(
            mock_bot, mock_message, temp_video_file, platform_name, 
            original_url, file_size_mb, True, mock_progress_msg
        )
    ...
    @pytest.mark.asyncio
    async def test_original_link_fallback(self, mock_bot, mock_message, mock_progress_msg, temp_video_file):
    ...
    @pytest.mark.asyncio
    async def test_complete_failure_troubleshooting(self, mock_bot, mock_message, mock_progress_msg, temp_video_file):
    ...
    async def test_end_to_end_with_compression_and_fallback(
        self, mock_compressor_class, mock_download, mock_requests, 
        mock_bot, mock_message, mock_progress_msg, large_temp_video_file
    ):
    ...
    @pytest.mark.asyncio
    @patch('handlers.social_media.utils.requests.post')
    async def test_api_error_handling(self, mock_requests, mock_bot, mock_message, mock_progress_msg):
    ...
    @pytest.mark.asyncio
    async def test_compression_error_handling(self, mock_bot, mock_message, mock_progress_msg, temp_video_file):
        ...
        with patch('handlers.social_media.utils.process_social_media_video') as mock_process:
            ...
            try:
                await mock_process(mock_message, mock_bot, original_url, platform_name, mock_progress_msg)
...
class TestSocialMediaCompressionIntegration:
    ...
    @patch('handlers.social_media.utils._download_video_to_temp')
    @patch('handlers.social_media.utils.VideoCompressor')
    async def test_tiktok_compression_workflow(self, mock_compressor_class, mock_download, mock_requests,
                                             mock_bot, mock_message, mock_progress_msg):
    ...
    @patch('handlers.social_media.utils._download_video_to_temp')
    @patch('handlers.social_media.utils.VideoCompressor')
    async def test_facebook_compression_failure_fallback(self, mock_compressor_class, mock_download, mock_requests,
                                                        mock_bot, mock_message, mock_progress_msg):
    ...
    async def test_twitter_small_video_no_compression(self, mock_should_compress, mock_download, mock_requests,
                                                     mock_bot, mock_message, mock_progress_msg):
...
class TestRealWorldVideoURLs:
    ...
    async def test_download_video_with_timeout(self, mock_get):
        ...
        with pytest.raises(Exception) as exc_info:
            await _download_video_to_temp(
                "https://example.com/slow_video.mp4", "TikTok", 12345
            )
    ...
    @pytest.mark.asyncio
    @patch('handlers.social_media.utils.requests.post')
    async def test_api_rate_limiting_handling(self, mock_requests, mock_bot, mock_message, mock_progress_msg):
    ...
    @pytest.mark.asyncio
    @patch('handlers.social_media.utils.requests.post')
    async def test_api_invalid_url_handling(self, mock_requests, mock_bot, mock_message, mock_progress_msg):
        ...
        # Execute
        await process_social_media_video(
            mock_message, mock_bot, "https://instagram.com/private_video", "Instagram", mock_progress_msg
        )
...
class TestConcurrentCompressionHandling:
    ...
    @patch('handlers.social_media.utils._download_video_to_temp')
    @patch('handlers.social_media.utils.VideoCompressor')
    async def test_concurrent_compression_requests(self, mock_compressor_class, mock_download, mock_requests,
                                                  mock_bot, mock_message_factory):
