...
class TestProgressiveCompressionStrategy:
    ...
    def test_calculate_output_dimensions_ensures_even_numbers(self, video_compressor):
        ...
        for orig_w, orig_h, max_w, max_h in test_cases:
            width, height = video_compressor._calculate_output_dimensions(orig_w, orig_h, max_w, max_h)
    ...
    @pytest.mark.asyncio
    async def test_progressive_compression_strategy_first_attempt_success(self, video_compressor, sample_video_info):
        ...
        with tempfile.NamedTemporaryFile(suffix='.mp4', delete=False) as input_file:
            with tempfile.NamedTemporaryFile(suffix='.mp4', delete=False) as output_file:
                ...
                with patch.object(video_compressor, '_attempt_compression', return_value=True):
                    with patch('utils.video_compression.get_file_size_mb', return_value=30.0):  # Under target
    ...
    @pytest.mark.asyncio
    async def test_progressive_compression_strategy_multiple_attempts(self, video_compressor, sample_video_info):
        ...
        with tempfile.NamedTemporaryFile(suffix='.mp4', delete=False) as input_file:
            with tempfile.NamedTemporaryFile(suffix='.mp4', delete=False) as output_file:
                ...
                with patch.object(video_compressor, '_attempt_compression', side_effect=mock_attempt_compression):
    ...
    @pytest.mark.asyncio
    async def test_progressive_compression_strategy_all_attempts_fail(self, video_compressor, sample_video_info):
        ...
        with tempfile.NamedTemporaryFile(suffix='.mp4', delete=False) as input_file:
            with tempfile.NamedTemporaryFile(suffix='.mp4', delete=False) as output_file:
                ...
                with patch.object(video_compressor, '_attempt_compression', return_value=True):
                    with patch('utils.video_compression.get_file_size_mb', return_value=60.0):  # Always too large
                        result = await video_compressor._progressive_compression_strategy(
                            input_file.name, output_file.name, 45.0, sample_video_info
                        )
    ...
    @pytest.mark.asyncio
    async def test_progressive_compression_strategy_progress_callback(self, video_compressor, sample_video_info):
        ...
        with tempfile.NamedTemporaryFile(suffix='.mp4', delete=False) as input_file:
            with tempfile.NamedTemporaryFile(suffix='.mp4', delete=False) as output_file:
                ...
                with patch.object(video_compressor, '_attempt_compression', side_effect=mock_attempt_compression):
                    with patch('utils.video_compression.get_file_size_mb', side_effect=mock_get_file_size_mb):
                        result = await video_compressor._progressive_compression_strategy(
                            input_file.name, output_file.name, 45.0, sample_video_info, progress_callback
                        )
        ...
        with tempfile.NamedTemporaryFile(suffix='.mp4', delete=False) as input_file:
            with tempfile.NamedTemporaryFile(suffix='.mp4', delete=False) as output_file:
                ...
                mock_video_info = VideoInfo(
                    ...
                    bitrate=5000000, codec='h264', size_mb=100.0
                )
                
                with patch.object(video_compressor, 'get_video_info', return_value=mock_video_info):
                    with patch.object(video_compressor, '_progressive_compression_strategy', return_value=True) as mock_strategy:
    ...
    def test_resolution_levels_progressive_downscaling(self, video_compressor, sample_video_info):
        ...
        for i in range(1, len(resolution_levels)):
            ...
            assert curr_pixels <= prev_pixels, f"Resolution level {i} has more pixels than level {i-1}"
