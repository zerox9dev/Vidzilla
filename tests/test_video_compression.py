...
class TestVideoCompressionEngine:
    ...
    @pytest.mark.asyncio
    async def test_compress_video_already_small(self, temp_video_file, temp_output_file, compression_config, mock_video_probe_data):
    ...
    @pytest.mark.asyncio
    async def test_estimate_compression_time(self, temp_video_file, compression_config, mock_video_probe_data):
    ...
    @pytest.mark.asyncio
    async def test_compress_video_with_progress_callback(self, temp_video_file, temp_output_file, compression_config, mock_video_probe_data):
...
class TestCompressionResultHandling:
    ...
    def test_create_compression_result_success(self, temp_video_file):
        ...
        from utils.video_compression import create_compression_result, create_temp_file, cleanup_temp_files
...
class TestVideoFormatsAndSizes:
        ...
        for size_mb in test_sizes:
            ...
            try:
                ...
                mock_video_info = VideoInfo(
                    width=1920, height=1080, duration=120.0, fps=30.0,
                    bitrate=5000000, codec='h264', size_mb=size_mb
                )
    ...
    async def test_compression_with_various_formats(self, video_compressor):
        ...
        for format_ext in formats:
            ...
            try:
                mock_video_info = VideoInfo(
                    width=1920, height=1080, duration=120.0, fps=30.0,
                    bitrate=5000000, codec='h264', size_mb=60
                )
    ...
    async def test_compression_with_different_resolutions(self, video_compressor):
        ...
        for width, height in resolutions:
            ...
            try:
                mock_video_info = VideoInfo(
                    width=width, height=height, duration=120.0, fps=30.0,
                    bitrate=5000000, codec='h264', size_mb=60
                )
...
class TestErrorScenariosAndEdgeCases:
    ...
    async def test_insufficient_disk_space_error(self, video_compressor):
        ...
        try:
            mock_video_info = VideoInfo(
                width=1920, height=1080, duration=120.0, fps=30.0,
                bitrate=5000000, codec='h264', size_mb=100
            )
    ...
    async def test_corrupted_video_file_error(self, video_compressor):
        ...
        try:
            with patch.object(video_compressor, 'get_video_info', side_effect=VideoCorruptedError("Corrupted file")):
                result = await video_compressor.compress_if_needed(temp_path, 50)
                assert result.success is False
    ...
    async def test_unsupported_format_error(self, video_compressor):
        ...
        try:
            with patch.object(video_compressor, 'get_video_info', side_effect=UnsupportedFormatError("Unsupported format")):
                result = await video_compressor.compress_if_needed(temp_path, 50)
                assert result.success is False
    ...
    async def test_very_short_video(self, video_compressor):
        ...
        try:
            ...
            mock_video_info = VideoInfo(
                width=1920, height=1080, duration=0.1, fps=30.0,
                bitrate=5000000, codec='h264', size_mb=60
            )
    ...
    async def test_very_long_video(self, video_compressor):
        ...
        try:
            ...
            mock_video_info = VideoInfo(
                width=1920, height=1080, duration=36000.0, fps=30.0,
                bitrate=5000000, codec='h264', size_mb=60
            )
...
class TestPerformanceAndTimeLimits:
    ...
    async def test_compression_time_estimation_accuracy(self, video_compressor):
        ...
        for width, height, duration, min_time, max_time in test_cases:
            ...
            try:
                mock_video_info = VideoInfo(
                    width=width, height=height, duration=duration, fps=30.0,
                    bitrate=5000000, codec='h264', size_mb=60
                )
    ...
    async def test_memory_usage_with_large_files(self, video_compressor):
        ...
        for i in range(3):
            ...
            try:
                mock_video_info = VideoInfo(
                    width=1920, height=1080, duration=120.0, fps=30.0,
                    bitrate=5000000, codec='h264', size_mb=200
                )
    ...
    async def test_concurrent_compression_handling(self, video_compressor):
        ...
        async def compress_single_file(file_size_mb: int) -> bool:
            ...
            try:
                mock_video_info = VideoInfo(
                    width=1920, height=1080, duration=120.0, fps=30.0,
                    bitrate=5000000, codec='h264', size_mb=file_size_mb
                )
...
class TestCompressionQualityAndSettings:
    ...
    async def test_quality_level_progression(self, video_compressor):
        ...
        try:
            ...
            async def mock_compress_video(input_path, output_path, target_size, quality_level=None, **kwargs):
                if quality_level:
            ...
            with patch.object(video_compressor, 'get_video_info', return_value=mock_video_info):
                with patch.object(video_compressor, 'compress_video', side_effect=mock_compress_video):
                    result = await video_compressor.compress_if_needed(temp_path, 50)
    ...
    async def test_resolution_downscaling(self, video_compressor):
        ...
        try:
            ...
            mock_video_info = VideoInfo(
                width=7680, height=4320, duration=120.0, fps=30.0,
                bitrate=50000000, codec='h264', size_mb=100
            )
