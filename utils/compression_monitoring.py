...
class CompressionStatsTracker:
    ...
    def _check_system_warnings(self, metrics: SystemMetrics):
        ...
        if warnings:
            ...
            if (COMPRESSION_MONITORING['enable_admin_notifications'] and 
                time.time() - self.last_admin_notification > 3600):  # Max 1 notification per hour
                self._send_admin_notification(warning_msg)
    
    def record_compression_start(self, user_id: Optional[int] = None, 
    ...
    def record_compression_result(self, metrics: CompressionMetrics):
        ...
        if (not metrics.success and 
            COMPRESSION_MONITORING['enable_admin_notifications'] and
            self.failure_count % COMPRESSION_MONITORING['admin_notification_threshold_failures'] == 0):
    ...
    def get_performance_stats(self, hours: int = 24) -> Dict[str, Any]:
        ...
        compression_ratios = [
            ...
            if m.success and m.compression_ratio
        ]
        ...
        if recent_system_metrics:
            system_performance = {
                'avg_cpu_usage': sum(m.cpu_usage_percent for m in recent_system_metrics) / len(recent_system_metrics),
                'avg_memory_usage': sum(m.memory_usage_percent for m in recent_system_metrics) / len(recent_system_metrics),
                'avg_disk_usage': sum(m.disk_usage_percent for m in recent_system_metrics) / len(recent_system_metrics),
                'min_available_disk_mb': min(m.available_disk_space_mb for m in recent_system_metrics),
                'max_active_compressions': max(m.active_compressions for m in recent_system_metrics),
                'avg_temp_dir_size_mb': sum(m.temp_directory_size_mb for m in recent_system_metrics) / len(recent_system_metrics)
            }
...
def log_compression_start(user_id: Optional[int] = None, 
                         original_size_mb: float = 0.0,
                         platform: Optional[str] = None):
...
def log_compression_result(user_id: Optional[int] = None,
                          original_size_mb: float = 0.0,
...
def log_cleanup_operation(files_cleaned: int, size_freed_mb: float):
    ...
    if files_cleaned > 0:
        compression_logger.info(
            ...
            f"{size_freed_mb:.2f}MB freed"
        )


def log_performance_metrics(operation: str, duration_seconds: float, 
                          file_size_mb: float, success: bool):
...
class CompressionOperationLogger:
    ...
    def set_result(self, success: bool, compressed_size_mb: Optional[float] = None,
                   compression_ratio: Optional[float] = None,
                   video_duration: Optional[float] = None,
                   video_resolution: Optional[str] = None,
                   compression_method: Optional[str] = None):
