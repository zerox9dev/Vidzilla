...
def check_disk_space(path: str, required_mb: float) -> bool:
    ...
    try:
        ...
        logger.info(f"Disk space check: {available_mb:.2f}MB available, {required_mb:.2f}MB required")
...
async def get_video_info(video_path: str) -> VideoInfo:
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Video info extracted successfully: {width}x{height}, {duration:.1f}s, {codec}")
            return video_info
...
def check_video_size_against_limit(video_path: str, size_limit_mb: float = 50.0) -> tuple[bool, float]:
    """
    ...
    logger.info(f"Video size check: {file_size_mb:.2f}MB {'>' if needs_compression else '<='} {size_limit_mb}MB limit")
            ...
            if os.path.exists(file_path):
                ...
                if os.path.isfile(file_path):
                    ...
                    try:
                        ...
                        compression_logger.info(f"Cleaned up temporary file: {file_path} ({file_size_mb:.2f}MB)")
                    ...
                    compression_logger.warning(f"Skipping directory cleanup (expected file): {file_path}")
                else:
                    compression_logger.warning(f"Unknown file type, skipping cleanup: {file_path}")
            else:
                compression_logger.debug(f"Temporary file already removed or doesn't exist: {file_path}")
                
        except Exception as e:
            compression_logger.error(f"Unexpected error cleaning up temporary file {file_path}: {str(e)}")
...
def cleanup_temp_directory(temp_dir: str, max_age_hours: int = 24) -> None:
    ...
    compression_logger.info(f"Cleaning up temporary directory: {temp_dir} (files older than {max_age_hours}h)")
            ...
            try:
                if os.path.isfile(file_path):
                    ...
                    if file_age > max_age_seconds:
                        ...
                        compression_logger.debug(f"Cleaned up old temp file: {filename} ({file_size_mb:.2f}MB, {file_age/3600:.1f}h old)")
...
def monitor_disk_usage(path: str, warning_threshold_percent: float = 90.0) -> dict:
    ...
    try:
        ...
        if usage_stats['warning']:
            logger.warning(f"Disk usage warning: {usage_percent:.1f}% used ({available_mb:.2f}MB available)")
        else:
...
def create_temp_file(suffix: str = '.mp4', prefix: str = 'compressed_', temp_dir: Optional[str] = None) -> str:
    """
...
class CompressionContext:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if self.temp_files:
            logger.info(f"Cleaning up {len(self.temp_files)} temporary files from compression context")
        ...
        if exc_type:
            logger.error(f"Compression context exited with exception: {exc_type.__name__}: {exc_val}")
...
class VideoCompressor:
    ...
    def _setup_temp_directory(self):
        ...
        if temp_dir:
            if not ensure_temp_directory(temp_dir):
                logger.warning(f"Cannot use configured temp directory {temp_dir}, falling back to system default")
    ...
    async def compress_if_needed(self, video_path: str, max_size_mb: float = 50.0, 
                               user_id: Optional[int] = None, platform: Optional[str] = None) -> CompressionResult:
        """
        ...
        compression_logger.info(f"Starting compression check for: {video_path} (limit: {max_size_mb}MB)")
        ...
        with CompressionOperationLogger(
            user_id=user_id,
            original_size_mb=initial_file_size,
            platform=platform
        ) as monitor:
            ...
            with self.get_compression_context() as ctx:
                try:
                    ...
                    needs_compression, file_size_mb = check_video_size_against_limit(video_path, max_size_mb)
                    
                    if not needs_compression:
                        # File is already within limit, no compression needed
                        compression_logger.info(f"Video {video_path} ({file_size_mb:.2f}MB) is already under {max_size_mb}MB limit - skipping compression")
                    ...
                    compression_logger.info(f"Video {video_path} ({file_size_mb:.2f}MB) exceeds {max_size_mb}MB limit - starting compression")
                    ...
                    try:
                        ...
                        compression_logger.warning(f"Could not extract video info for monitoring: {e}")
                    ...
                    if compression_success and validate_file_size(temp_output, max_size_mb):
                        ...
                        compression_ratio = calculate_compression_ratio(file_size_mb, compressed_size_mb)
                        
                        compression_logger.info(f"Compression successful: {file_size_mb:.2f}MB -> {compressed_size_mb:.2f}MB")
                        ...
                        monitor.set_disk_space(disk_stats['available_mb'], final_disk_stats['available_mb'])
                ...
                except (FFmpegNotFoundError, CompressionTimeoutError, InsufficientDiskSpaceError, 
                        UnsupportedFormatError, VideoCorruptedError, CompressionQualityError) as e:
                    ...
                    compression_logger.error(f"Unexpected error in compress_if_needed for {video_path}: {str(e)}", exc_info=True)
    ...
    async def compress_video(self, input_path: str, output_path: str, target_size_mb: float, 
        ...
        """
        logger.info(f"Starting video compression: {input_path} -> {output_path} (target: {target_size_mb}MB)")
        ...
        try:
            ...
            logger.info(f"Video info: {video_info.width}x{video_info.height}, {video_info.duration:.1f}s, {video_info.size_mb:.2f}MB")
            
            # If video is already smaller than target, just copy it
            if video_info.size_mb <= target_size_mb:
                logger.info(f"Video already under target size ({video_info.size_mb}MB <= {target_size_mb}MB)")
        ...
        except (FFmpegNotFoundError, VideoCorruptedError, UnsupportedFormatError, CompressionTimeoutError):
            # Re-raise our custom exceptions
            raise
        except Exception as e:
    ...
    async def _progressive_compression_strategy(self, input_path: str, output_path: str, 
                                              target_size_mb: float, video_info: VideoInfo,
                                              progress_callback: Optional[Callable[[float], None]] = None) -> bool:
        """
        ...
        logger.info(f"Will try {total_attempts} compression attempts across {len(valid_resolution_levels)} resolution levels")
        
        # Try each resolution level with each quality level
        for res_idx, (max_width, max_height) in enumerate(valid_resolution_levels):
            logger.info(f"Trying resolution level {res_idx + 1}/{len(valid_resolution_levels)}: {max_width}x{max_height}")
            
            for quality_idx, crf in enumerate(quality_levels):
                current_attempt += 1
                logger.info(f"Compression attempt {current_attempt}/{total_attempts}: CRF {crf} at max {max_width}x{max_height}")
                ...
                if output_width * output_height < min_acceptable_pixels:
                    logger.warning(f"Skipping attempt with too small resolution: {output_width}x{output_height}")
                    continue
                ...
                try:
                    ...
                    if success:
                        ...
                        if output_size_mb <= target_size_mb:
                            ...
                            logger.info(f"Progressive compression successful: {video_info.size_mb:.2f}MB -> {output_size_mb:.2f}MB (ratio: {compression_ratio:.2f})")
                            logger.info(f"Final settings: CRF {crf}, resolution {output_width}x{output_height}")
                            return True
                        else:
                    ...
                    last_error = e
                    logger.error(f"Unexpected error during compression attempt {current_attempt}: {str(e)}", exc_info=True)
                    continue
        
        # If all progressive attempts failed
        logger.error(f"All {total_attempts} progressive compression attempts failed for {input_path}")
        ...
        if timeout_count > 0:
            ...
            raise CompressionQualityError("Unable to compress video to target size with acceptable quality")
    
    def _calculate_output_dimensions(self, original_width: int, original_height: int, 
    ...
    async def _attempt_compression(self, input_path: str, output_path: str, crf: int,
                                 output_width: int, output_height: int, timeout_seconds: int) -> bool:
        """
        ...
        """
        logger.info(f"Attempting compression: CRF {crf}, {output_width}x{output_height}, timeout {timeout_seconds}s")
        ...
        except (CompressionTimeoutError, FileNotFoundError, UnsupportedFormatError, CompressionError):
            # Re-raise our custom exceptions
            raise
        except Exception as e:
    ...
    async def estimate_compression_time(self, video_path: str) -> int:
        ...
        try:
            ...
            estimated_time = base_time * resolution_factor * duration_factor * codec_factor * fps_factor
            
            # Add some buffer and ensure minimum/maximum bounds
            estimated_time = max(10, min(600, int(estimated_time * 1.3)))  # 10s to 10min range
            
            logger.info(f"Compression time estimation: {estimated_time}s (base: {base_time:.1f}s, "
                       f"resolution: {resolution_factor}x, duration: {duration_factor:.1f}x, "
                       f"codec: {codec_factor}x, fps: {fps_factor}x)")
            
            return estimated_time
            
        except (FileNotFoundError, VideoCorruptedError, UnsupportedFormatError, FFmpegNotFoundError):
            # Re-raise our custom exceptions
            raise
        except Exception as e:
