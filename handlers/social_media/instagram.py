    ...
    try:
        ...
        if "/reel/" in instagram_url or "/p/" in instagram_url:
            try:
                if progress_msg:
                    await safe_edit_message(
                        progress_msg, "⏳ Processing Instagram link... 50%", "Instagram"
                    )

                logger.info(
                    f"""Attempting to download video from: {
                            instagram_url}"""
                )
                ...
                if progress_msg:
                    await safe_edit_message(
                        progress_msg, "⏳ Processing Instagram link... 75%", "Instagram"
                    )
                ...
                if not video_files:
                    ...
                    if image_files:
                        if progress_msg:
                            await safe_edit_message(
                                progress_msg, "⏳ Processing Instagram link... 90%", "Instagram"
                            )
                ...
                if should_compress_video(video_path, max_size_mb=50.0):
                    ...
                    try:
                        ...
                        if compression_result.success and compression_result.compressed_path:
                            ...
                            logger.info(
                                f"Compression successful: {compression_result.original_size_mb:.2f}MB -> {compression_result.compressed_size_mb:.2f}MB ({ratio_percent}% reduction)"
                            )
                        else:
                            # Compression failed, use original file
                            logger.warning(
                                f"Compression failed: {compression_result.error_message}"
                            )
                            if progress_msg:
                                fallback_msg = COMPRESSION_MESSAGES["fallback"]
                                detailed_fallback = f"{fallback_msg}\n📁 Original size: {file_size_mb:.1f}MB\n⏳ Sending as document..."
                                await safe_edit_message(
                                    progress_msg, detailed_fallback, "Instagram"
                                )
                    ...
                    logger.info(
                        f"Video size ({file_size_mb:.2f}MB) is within 50MB limit - no compression needed"
                    )
                    if progress_msg:
                ...
                try:
                    ...
                    send_result = await bot.send_video(
                        chat_id=message.chat.id, video=video_file, **video_params
                    )
                ...
                logger.error(f"Error downloading or processing video: {str(e)}")

                # Check if this is an Instagram rate limiting error (401 Unauthorized)
                error_message = str(e)
                if (
                    "401 Unauthorized" in error_message
                    and "Please wait a few minutes before you try again" in error_message
                ):
                    if progress_msg:
                        await safe_edit_message(
                            progress_msg,
                    ...
                    if progress_msg:
                        await safe_edit_message(
                            progress_msg,
                            f"❌ Video compression error: {str(e)}\nTry again or contact support if the issue persists.",
                            "Instagram",
                        )
                    else:
                        await bot.send_message(
                    ...
                    if progress_msg:
                        await safe_edit_message(
                            progress_msg, f"❌ Error downloading video: {str(e)}", "Instagram"
                        )
                    else:
                        await bot.send_message(
                            message.chat.id, f"Error downloading video: {str(e)}"
                        )
        else:
            ...
            if progress_msg:
                await safe_edit_message(
                    progress_msg,
                    "❌ Invalid Instagram URL. Please provide a link to a post or reel.",
                    "Instagram",
                )
            else:
                await bot.send_message(
        ...
        if (
            ...
            if progress_msg:
                await safe_edit_message(
                    progress_msg, f"❌ Error processing Instagram video: {str(e)}", "Instagram"
                )
            else:
                await bot.send_message(
                    message.chat.id, f"Error processing Instagram video: {str(e)}"
                )
    finally:
