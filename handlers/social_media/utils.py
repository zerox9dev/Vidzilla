    ...
    try:
        ...
        if False:
            ...
            if should_compress_video(temp_video_path, max_size_mb=50.0):
                logger.info(
                    f"{platform_name} video size ({file_size_mb:.2f}MB) exceeds 50MB limit - starting compression"
                )
                ...
                if progress_msg:
                    ...
                    await safe_edit_message(
                        progress_msg, f"⏳ {platform_name}: {compression_msg}", platform_name
                    )
                ...
                try:
                    ...
                    logger.warning(
                        f"Could not estimate compression time for {platform_name}: {est_error}"
                    )
                ...
                try:
                    ...
                    if compression_result.success and compression_result.compressed_path:
                        ...
                        ratio_percent = (
                            int((1 - compression_result.compression_ratio) * 100)
                        ...
                        logger.info(
                            f"{platform_name} compression successful: {compression_result.original_size_mb:.2f}MB -> {compression_result.compressed_size_mb:.2f}MB ({ratio_percent}% reduction)"
                        )
                    else:
                        # Compression failed, use original file
                        logger.warning(
                            f"{platform_name} compression failed: {compression_result.error_message}"
                        )
                        if progress_msg:
                    ...
                    if progress_msg:
                        error_msg = COMPRESSION_MESSAGES["error"].format(
                            error=str(compression_error)
                        )
                ...
                logger.info(
                    f"{platform_name} video size ({file_size_mb:.2f}MB) is within 50MB limit - no compression needed"
                )
                if progress_msg:
        ...
        if "compression" in error_message.lower():
            ...
            if progress_msg:
                await safe_edit_message(
                    progress_msg,
                    f"❌ Failed to download {platform_name} video: {str(e)}\nThe video may be private or unavailable.",
                    platform_name,
                )
            else:
                await bot.send_message(
            ...
            if progress_msg:
                await safe_edit_message(
                    progress_msg,
                    f"❌ Error processing {platform_name} video: {str(e)}",
                    platform_name,
                )
            else:
                await bot.send_message(
                    message.chat.id, f"Error processing {platform_name} video: {str(e)}"
                )
        ...
        if temp_video_path and os.path.exists(temp_video_path):
            try:
                ...
                logger.warning(
                    f"Failed to clean up temporary file {temp_video_path}: {cleanup_error}"
                )

        if (
            compressed_video_path
            and compressed_video_path != temp_video_path
            and os.path.exists(compressed_video_path)
        ):
            try:
                ...
                logger.warning(
                    f"Failed to clean up compressed file {compressed_video_path}: {cleanup_error}"
                )
...
async def _attempt_video_delivery_with_fallbacks(
    bot,
    message,
    video_path: str,
    platform_name: str,
    ...
    try:
        if progress_msg:
            await safe_edit_message(
                progress_msg, f"⏳ Sending {platform_name} video...", platform_name
            )
    ...
    try:
        if progress_msg:
            await safe_edit_message(
                progress_msg, f"⚠️ Sending {platform_name} video as document...", platform_name
            )
    ...
    try:
        if progress_msg:
            await safe_edit_message(
                progress_msg, f"⚠️ Providing {platform_name} download link...", platform_name
            )
        ...
        await bot.send_message(
            chat_id=message.chat.id, text=fallback_message, disable_web_page_preview=False
        )
        ...
        if progress_msg:
            await safe_edit_message(
                progress_msg,
                f"⚠️ {platform_name} video link provided (delivery failed)",
                platform_name,
            )
    ...
    try:
        ...
        await bot.send_message(
            chat_id=message.chat.id, text=error_message, disable_web_page_preview=True
        )

        if progress_msg:
            await safe_edit_message(
                progress_msg,
                f"❌ {platform_name} video delivery failed - see troubleshooting message",
                platform_name,
            )
...
async def detect_platform_and_process(message, bot, url, progress_msg=None):
    ...
    if "instagram.com" in url:
        if progress_msg:
            await safe_edit_message(
                progress_msg, "⏳ Processing Instagram link... 25%", "Instagram"
            )
