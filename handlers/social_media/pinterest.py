...
async def process_pinterest(message, bot, pinterest_url, progress_msg=None):
    try:
        if progress_msg:
            await progress_msg.edit_text("⏳ Processing Pinterest link... 50%")
        ...
        if progress_msg:
            await progress_msg.edit_text("⏳ Processing Pinterest link... 75%")

        if response.status_code == 200 and 'medias' in data and len(data['medias']) > 0:
            video_url = data['medias'][0]['url']
            
            if progress_msg:
                await progress_msg.edit_text("⏳ Processing Pinterest link... 90%")
            ...
            if progress_msg:
                await progress_msg.edit_text("✅ Pinterest video processed successfully! 100%")
        else:
        ...
        if progress_msg:
            await progress_msg.edit_text(f"❌ Error processing Pinterest video: {str(e)}")
        else:
