...
async def process_youtube(message, bot, youtube_url, progress_msg=None):
    try:
        if progress_msg:
            await progress_msg.edit_text("⏳ Processing YouTube link... 50%")
        ...
        if progress_msg:
            await progress_msg.edit_text("⏳ Processing YouTube link... 75%")

        if response.status_code == 200 and 'medias' in data and len(data['medias']) > 0:
            video_url = data['medias'][0]['url']
            
            if progress_msg:
                await progress_msg.edit_text("⏳ Processing YouTube link... 90%")
            ...
            file_name = f"youtube_video{message.from_user.id}.mp4"
            ...
            if progress_msg:
                await progress_msg.edit_text("✅ YouTube video processed successfully! 100%")
        else:
        ...
        if progress_msg:
            await progress_msg.edit_text(f"❌ Error processing YouTube video: {str(e)}")
        else:
