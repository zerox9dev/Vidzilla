...
async def process_facebook(message, bot, facebook_url, progress_msg=None):
    try:
        if progress_msg:
            await progress_msg.edit_text("⏳ Processing Facebook link... 50%")
        ...
        if progress_msg:
            await progress_msg.edit_text("⏳ Processing Facebook link... 75%")

        if response.status_code == 200 and 'medias' in data and len(data['medias']) > 0:
            video_url = data['medias'][0]['url']
            
            if progress_msg:
                await progress_msg.edit_text("⏳ Processing Facebook link... 90%")
            ...
            if progress_msg:
                await progress_msg.edit_text("✅ Facebook video processed successfully! 100%")
        else:
        ...
        if progress_msg:
            await progress_msg.edit_text(f"❌ Error processing Facebook video: {str(e)}")
        else:
